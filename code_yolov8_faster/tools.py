...
def compute_color_for_labels(label):
    ...
    color = [int((p * (label ** 2 - label + 1) + 255) % 255) for p in palette]

    # Adjust the color values to make it more red
    color[2] = 255  # Set the red component to maximum
    # color[1] = int(color[1] * 0.5)  # Reduce the green component
...
def draw_info(frame, results):
    for i, bbox in enumerate(results):
        ...
        # 彩框 yolov5
        cv2.rectangle(frame, (int(box[0]), int(box[1])), (int(box[2]), int(box[3])), color, 4)
        # cls_name
        cv2.putText(frame, str(cls_name), (int(box[0] + 20), int(box[1] + 50)), 0, 2, color, 3)
